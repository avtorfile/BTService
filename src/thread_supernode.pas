...
uses
  ...
  const_commands,registry,utility_ares,ares_types,classes2,
  keywfunc, class_cmdlist, types_supernode, dialogs, 
  const_ares,helper_supernode_crypt,hashlist,uExtraData;
...
uses
  ...
  helper_ares_nodes,const_udpTransfer;
...
begin
 ...
 try
   ...
   my_tcp_port := Ini.ReadInteger('Network','SupernodePort', 0)
 finally
   ini.UpdateFile;
   ini.Free;
 end;
...
str_myagent:=AGENT_NAME+' '+versioneares+CHRNULL;
      ...
      checksync;
       ...
       try
         if not setted_preferred_port then
          if user_list.count>30 then
          begin
            EnterCriticalSection(ServiceSection);
            try
              {synchronize}(set_reg_preferred_port);
            finally
              LeaveCriticalSection(ServiceSection);
            end;              
          end;
  ...
  GlobUser.shared_list.add(sharef); // add file in userlist
            ...
            inc(GlobUser.shared_Size,fsize);
              ...
              if sharef^.amime=3 then
               if sharef^.size>734003200 then begin
                inc(GlobUser.NumbigVideos);

                   if GlobUser.NumBigVideos>30 then
                      if GlobUser.shared_size>32212254720 then begin
                        GlobUser.shareBlocked:=true;
                        //check_user_video;
