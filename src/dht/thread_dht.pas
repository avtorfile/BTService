...
uses
 classes,classes2,sysutils,dhtconsts,dhttypes,windows,
 dhtzones,blcksock,synsock,zlibex,int128,math,dhthashlist,
 dhtcontact,dhtsearchManager,activex{coinitialize},hashlist,keywfunc,
 types_supernode,class_cmdlist,ares_types,ares_objects,uExtraData, dialogs;
...
uses
 ...
 {helper_visual_headers,}mysupernodes;
...
begin
 ...
 if DHT_buffer[0]<>OP_DHT_HEADER then begin
    ...
    if DHT_buffer[0]=OP_DHT_PACKEDPROT then begin
        try
          ZDecompress(@DHT_buffer[2],DHT_len_recvd-2,buff,outsize);
//        if ZDecompress(@DHT_buffer[2],DHT_len_recvd-2,buff,outsize) then
           begin
...
if DHT_len_recvd<3 then exit;
 ...
 if m_numFirewallResults>=3 then
  if m_notFirewalledMessages>=3 then
  begin
    EnterCriticalSection(ServiceSection);
    try
      {synchronize}(SyncNotFirewalled);
    finally
      LeaveCriticalSection(ServiceSection);
    end;
  end;
...
if wanted_search.strict1 then DHT_ParseComplexField(complex);
...
if not wanted_search.strict1 then begin
...
if wanted_search.strict1 then begin
...
if ((m_lastContact=0) or
    (nowt-m_lastContact>DHT_DISCONNECTDELAY)) then
     if nowt-lastBootstrap>60 then
     begin
       EnterCriticalSection(ServiceSection);
       try
         {synchronize}(check_bootstrap);
       finally
         LeaveCriticalSection(ServiceSection);
       end;       
     end;
