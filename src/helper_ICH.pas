...
uses
 ...
 {tntwindows,}ares_objects,synsock,dialogs;
...
uses
 ...
 helper_unicode,securehash,helper_ipfunc,
 helper_preview;
...
begin
      ...
      if not FileExists(data_path+'\Data\PHashIdxTemp.dat') then stream_to:=MyFileOpen(data_path+'\Data\PHashIdxTemp.dat',ARES_OVERWRITE_EXISTING)
...
begin
 Tnt_createdirectoryW(pwidechar(data_path+'\Data\TempDL'),nil);
...
result:=0;
    ...
    if not FileExists(data_path+'\Data\PHashIdx.dat') then stream_to:=MyFileOpen(data_path+'\Data\PHashIdx.dat',ARES_OVERWRITE_EXISTING)
...
move(UDP_Buffer^,buffer[0],LenBuffer);
 ...
 if not FileExists(fname) then stream_to:=ICH_ExtractDataForUpload(hash_sha1,insertion_point,fname)
    ...
    position:=stream_from.position;
     ...
     while (stream_from.position<>position-36) do begin
      ...
      sleep(50);
       ...
       Tnt_createdirectoryW(pwidechar(data_path+'\Data\TempUL'),nil);
