...
uses
 ares_objects,sysutils,{comettrees,}classes2,classes,windows;
...
nomefile:=utf8strtowidestr(data_search^.filenameS);
 ...
 if data_search^.fsize<MEGABYTE then begin
  ...
  if data_search^.amime=ARES_MIME_MP3 then begin
     ...
     if length(data_search^.url)>0 then lista.add(GetLangStringA(STR_URL)+': '+data_search^.url);
  end else
...
if ((data_upload^.progress<4096) and (data_upload^.size<4096)) then begin
 size_progress:=format_currency(data_upload^.progress)+' '+STR_BYTES;
end else
if (data_upload^.progress)<MEGABYTE then begin
 kbyt:=(data_upload^.progress)/KBYTE;
...
if ((data_upload^.velocita>0) and (not data_upload^.completed)) then begin
 ...
 progress:=(data_upload^.progress);
 if (data_upload^.size)=0 then progress:=1 else progress:=progress/(data_upload^.size);
 progress:=progress*100;
 percent:=FloatToStrF(progress, ffNumber, 18, 2);

 size_chunk:=(data_upload^.size);
...
begin
 ...
 if aval<20 then result:=GetLangStringW(STR_GOOD)+' ('+inttostr(aval)+')' else
