...
uses
 classes,classes2,btcore,uExtraData{,tntwindows};
...
uses
 helper_diskio,windows,ares_objects,helper_unicode,
 sysutils,vars_global,helper_strings,BitTorrentUtils;
...
var
 ...
 fileOffset:int64;
 filname,rootpath,parseStr,payload,therootpath:string;
...
for i:=0 to high(Transfer.fPieces) do begin
 ...
 chunk:=TBitTorrentChunk.create(Transfer,chunkOffset,chunkSize,i,Transfer.fprogressive);
...
if ((not FileExists(dbname)) or (dbstream=nil)) then dbstream:=MyFileOpen(dbname,ARES_OVERWRITE_EXISTING);
