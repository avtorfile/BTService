...
uses
ares_objects,classes,windows,sysutils,thread_download;
...
uses
 helper_strings,helper_unicode{,tntwindows},
...
if FileExists(utf8strtowidestr(download.filename)) then helper_diskio.deletefileW(utf8strtowidestr(download.filename)); // nel caso ci fosse già il filename in extremis!!
...
if not Tnt_MoveFileW(pwidechar(old_filename), pwidechar(utf8strtowidestr(download.filename))) then begin
  aggiunta:=inttostr(random(500));
   ...
   iterations:=0;
   while (not Tnt_MoveFileW(pwidechar(old_filename),pwidechar(utf8strtowidestr(download.filename)))) do begin
    aggiunta:=inttostr(random(500));
...
try       //   STANDARD_RIGHTS_WRITE   WRITE_OWNER
 ...
 if not FileExists(utf8strtowidestr(download.filename)) then begin
